...
# Forked from https://github.com/tensorflow/tensorflow/blob/master/tensorflow/contrib/training/python/training/hparam.py  pylint: disable=line-too-long
# Merged functions from https://github.com/tensorflow/tensorflow/blob/master/tensorflow/python/util/compat.py
...
class HParams(object):
    ...
    for name, value in hparam_def.hparam.items():
      ...
      if kind.endswith('_value'):
        ...
        if kind.startswith('int64'):
          ...
          self.add_hparam(name, as_str(getattr(value, kind)))
        else:
        ...
        if kind.startswith('int64'):
          ...
          # Setting attribute value to be 'str' to ensure the type is compatible
          # with both Python2 and Python3. UTF-8 encoding is assumed.
          self.add_hparam(name, [as_str(v) for v in getattr(value, kind).value])
        else:
  ...
  def set_hparam(self, name, value):
    ...
    elif isinstance(value, dict) and is_list:
      for k, v in value.items():
        getattr(self,
                name)[k] = _cast_to_type_if_compatible(name, param_type, v)
    else:
      if is_list:
        raise ValueError(
    ...
    for name in self._hparam_types:
      ...
      if is_list:
        if kind.startswith('bytes'):
          v_list = [as_bytes(v) for v in getattr(self, name)]
        else:
